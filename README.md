...
**Fix:** Change position attribute in index.css for RampInputSelect--dropdown-container from fixed to absolute. 
...
**Fix:** Since the checkbox uses a custom design, we need to have the checkbox input be invisible and on top of the custom design. Again, changed attributes in index.css to achieve this. 
...
**Fix:** We need to call loadAllTransactions instead of loadTransactionsByEmployee when All Employees is selected. To do so, I added a check in App.tsx to call loadAllTransactions if All Employees is selected. 
...
**Fix:** We need to handle appending in the pagination logic. We modify the setPaginatedTransactions function to do so. 
...
**Fix:** We can set isLoading to false as soon as the employees list is fetched. We move that up a few lines in App.tsx -> loadAllTransactions.
...
**Fix:** Added 2 more checks to the isDisabled condition for the View More Button:
1. If the paginatedTransactions.nextPage is null
2. If the user selected anything other than All Employees
...
**Fix:** The cache for one view becomes outdated if an approval is checked in another view (view=filter by certain employee or all employees). We clear the cache whenever an approval is checked so that the most up to date values can be retrieved when we switch to another view that includes the transaction we approved. 
